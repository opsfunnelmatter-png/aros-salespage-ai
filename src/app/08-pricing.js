...
export default function PricingMatrix({ dict, lang = 'bm' }) {
  ...
  return (
    ...
    {/* DESIGN ALTERNATIVE: V2 */}
    <PricingSectionV2 dict={dict} lang={lang} />
    </>
  );
}

function PricingSectionV2({ dict, lang = 'bm' }) {
  const paymentLink = "https://buy.stripe.com/cNi9ALgQe23cdWufyj4ZG0n";
  const [promoDate, setPromoDate] = React.useState('');
  React.useEffect(() => {
    const today = new Date();
  ...
  return (
    <section id="pricing_section_v2" className="relative z-10 w-full bg-[#0B0F19] py-24 text-white overflow-hidden border-t border-white/10">
      {/* BACKGROUND LAYER */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff03_1px,transparent_1px),linear-gradient(to_bottom,#ffffff03_1px,transparent_1px)] bg-[size:24px_24px] pointer-events-none" />
      <div className="absolute top-1/4 left-1/4 w-[600px] h-[400px] bg-[radial-gradient(circle_at_center,rgba(249,115,22,0.03)_0%,transparent_70%)] pointer-events-none" />
      
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        
        {/* PROMO DATE BANNER AT THE TOP */}
        {promoDate && (
          <div className="mb-12 bg-gradient-to-r from-orange-500/5 to-amber-500/5 border border-orange-500/20 rounded-2xl p-5 shadow-[0_0_30px_rgba(249,115,22,0.05)] relative overflow-hidden max-w-4xl mx-auto">
            <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-orange-500 to-amber-500" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-center text-left pl-2">
              <div className="space-y-1">
                <span className="inline-flex items-center gap-1.5 text-[10px] font-mono font-black text-orange-400 uppercase tracking-widest bg-orange-500/10 px-2.5 py-1 rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          ...
          {/* KOLUM KIRI: Header & Specs */}
          <div className="space-y-8 text-left">
            
            {/* Tajuk Utama & Sub-penerangan */}
            ...
            {/* Kotak Jingga "Ini Bukan Chatbot Biasa" */}
            <div className="p-5 bg-orange-500/5 border border-orange-500/20 rounded-2xl">
              <div className="text-sm font-bold text-orange-400 mb-2">{dict.bot_diff_title}</div>
              <p className="text-xs text-neutral-400 leading-relaxed font-medium">{dict.bot_diff_desc}</p>
            </div>

            {/* Senarai Specifications */}
            <div>
              <div className="text-[9px] font-mono text-orange-500/80 uppercase tracking-widest mb-4">{pkg.spec_label}</div>
          ...
          {/* KOLUM KANAN: Unified Conversion Wrapper Card */}
          <div className="relative">
            {/* RECOMMENDED BADGE */}
            <div className="absolute -top-3.5 left-6 bg-gradient-to-r from-orange-500 to-amber-500 text-black text-[9px] font-mono font-black px-4 py-1.5 rounded-full uppercase tracking-wider shadow-[0_0_20px_rgba(249,115,22,0.4)] whitespace-nowrap z-20 flex items-center gap-1.5">
              <span className="relative flex h-1.5 w-1.5">
            ...
            <div className="bg-[#0F1424]/90 border-2 border-transparent rounded-3xl p-6 sm:p-8 shadow-[0_20px_50px_rgba(249,115,22,0.15)] relative overflow-hidden text-left space-y-6 z-10">
              
              {/* a) Box Info Harga */}
              <div className="bg-black/40 border border-orange-500/20 rounded-xl p-5 font-mono">
                <span className="text-[10px] text-red-500/80 line-through block font-black mb-0.5">{pkg.value_price}</span>
                <span className="text-[8px] text-neutral-500 font-bold block mb-1 uppercase tracking-widest">{pkg.investment_label}</span>
                <div className="flex items-baseline gap-0.5 mb-2">
                  <span className="text-3xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400 tracking-tight">{pkg.price}</span>
              ...
              {/* b) Jadual Breakdown Nilai Komponen */}
              <div className="bg-black/20 border border-white/5 rounded-2xl p-4">
                <div className="text-[9px] font-mono font-bold text-neutral-500 uppercase tracking-widest mb-3">{pkg.breakdown_label}</div>
                <div className="space-y-2">
                  {pkg.breakdown_items?.map((item, i) => (
              ...
              <button
                onClick={() => { window.location.href = paymentLink; }}
                className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-black hover:shadow-[0_0_25px_rgba(249,115,22,0.4)] hover:scale-[1.02] font-black py-3 rounded-xl transition-all duration-200 transform active:scale-95 cursor-pointer text-xs uppercase tracking-wider text-center block flex items-center justify-center gap-2"
              >
                <Lock className="w-3.5 h-3.5" />
                {pkg.btn_cta}
              </button>

              {/* d) Jaminan 72 Jam & Done-For-You (DFY) */}
              <div className="space-y-3">
                {/* 72-Hour Guarantee */}
                <div className="flex items-start gap-3 p-3.5 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                  <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                  <div>
                    <div className="text-[11px] font-bold text-emerald-400">{dict.guarantee_title}</div>
                    <div className="text-[10px] text-neutral-400 leading-relaxed mt-0.5">{dict.guarantee_desc}</div>
                  </div>
                </div>
                {/* DFY Block */}
                <div className="p-3.5 rounded-xl bg-zinc-900/40 border border-zinc-800/80">
                  <div className="flex items-center gap-2 mb-1.5 text-emerald-400 font-semibold text-xs tracking-wide uppercase">
              ...
              {/* e) Footer kad */}
              <div className="text-center flex flex-wrap items-center justify-center gap-1.5 text-[8px] sm:text-[9px] font-mono text-neutral-500 select-none pt-2">
                <span>{dict.secure_payment_label}</span>
                <span>•</span>
                <span>Secure Stripe Checkout</span>
                <span>•</span>
        ...
        {/* FEE FAQ BLOCK */}
        <div className="max-w-4xl mx-auto mt-12 bg-white/[0.02] border border-white/10 rounded-2xl p-5 flex items-start gap-3 text-left">
          <div className="w-5 h-5 shrink-0 mt-0.5">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="text-orange-400 w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
